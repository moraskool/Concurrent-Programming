...
{    
    ...
    extern __shared__ uchar sharedRGB[]; // Declare shared memory array to store RGB values
    int num_image_pixels = d_rows * d_columns;
    int threadId = blockDim.x * blockIdx.x + threadIdx.x;

    // Store the current thread's pixel values in shared memory
    sharedRGB[threadIdx.x * 3] = r[threadId];
    sharedRGB[threadIdx.x * 3 + 1] = g[threadId];
    sharedRGB[threadIdx.x * 3 + 2] = b[threadId];
    ...
    if(threadId < num_image_pixels)
    {
        ...
        uchar leftR, leftG, leftB;
        uchar rightR, rightG, rightB;
        int numChannels = 3; // Assuming RGB (3 channels) for each pixel
        ...
        // 1 Handle edge cases when the current thread is at the left or right edge of the input image
        if (threadIdx.x == 0)
        {
            // Left edge case, use the current pixel's value for the left side
            leftR = r[threadId];
            leftG = g[threadId];
            leftB = b[threadId];
        }
        ...
        {
            // Use the left neighbor's value for the left side
            leftR = sharedRGB[(threadIdx.x - 1) * numChannels];
            leftG = sharedRGB[(threadIdx.x - 1) * numChannels + 1];
            leftB = sharedRGB[(threadIdx.x - 1) * numChannels + 2];
        }

        if (threadIdx.x == blockDim.x - 1)
        {
            // Right edge case, use the current pixel's value for the right side
            rightR = r[threadId];
            rightG = g[threadId];
            rightB = b[threadId];
        }
        ...
        {
            // Use the right neighbor's value for the right side
            rightR = sharedRGB[(threadIdx.x + 1) * numChannels];
            rightG = sharedRGB[(threadIdx.x + 1) * numChannels + 1];
            rightB = sharedRGB[(threadIdx.x + 1) * numChannels + 2];
        }

        // Average the RGB values of the left and right neighbors with the current pixel's value
        uchar avgR = (leftR + sharedRGB[threadIdx.x * numChannels] + rightR) / 3;
        uchar avgG = (leftG + sharedRGB[threadIdx.x * numChannels + 1] + rightG) / 3;
        uchar avgB = (leftB + sharedRGB[threadIdx.x * numChannels + 2] + rightB) / 3;

        // Update the output arrays with the averaged RGB values
        r[threadId] = avgR;
        g[threadId] = avgG;
        b[threadId] = avgB;
